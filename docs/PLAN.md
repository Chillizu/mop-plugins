...
| D12 | 记忆三级：全局记忆（用户偏好/系统信息/系统级改动）、项目记忆（进展/思路/长期需求）、recall（`mop_recall` 兼容工具；0.2 默认以 DSH `sessionQuery.filterSessions/filterEvents` 为数据面，旧 zstd scanner 仅 fallback） | [memory-design](docs/design/memory-design.md) |
