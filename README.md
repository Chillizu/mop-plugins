...
- `mop_spawn_executor` `timeoutMs`：可选（毫秒），缺省无超时。超时经 AbortController 中止子代理并返回 `[aborted] executor timed out after {N}ms`（末尾仍带 `[executor-session: {id}]`）。本适配层不实现子代理 runtime：实际 child 生命周期由 DSH `ctx.subagents` 提供；run 一旦发布，无论成功、失败、取消或超时都会调用并等待原生 `run.dispose()` 进入 quiescence。限制：timeout 仍是 per-call 参数，无 Config 级默认。
